...
/*!
    ...
    \return 0 Returned on successfully generating the key and and storing it in the given curve25519_key structure
    \return ECC_BAD_ARG_E Returned if rng or key evaluate to NULL, or the input keysize does not correspond to the keysize for a curve25519 key ( 32 bytes)
    \return RNG_FAILURE_E Returned if the rng internal status is not DRBG_OK or if there is in generating the next random block with rng
    ...
    \param key pointer to the curve25519_key structure in which to store the generated key
...
/*!
    ...
    \return BAD_FUNC_ARG Returned if any of the input parameters passed in are NULL
    \return ECC_BAD_ARG_E Returned if the first bit of the public key is set, to avoid implementation fingerprinting
    
    \param private_key pointer to the curve25519_key structure initialized with the user’s private key
    \param public_key pointer to the  curve25519_key structure containing the received public key
    \param out pointer to a buffer in which to store the 32 byte computed secret key
    \param outlen pointer in which to store the length written to the output buffer
...
/*!
    ...
    \param out pointer to a buffer in which to store the 32 byte computed secret key
    \param outlen pointer in which to store the length written to the output buffer
    \param endian EC25519_BIG_ENDIAN or EC25519_LITTLE_ENDIAN to set which form to use.
    ...
    if ( wc_curve25519_shared_secret_ex(&privKey, &pubKey, sharedKey, &keySz, EC25519_BIG_ENDIAN) != 0 ) {
	    // error generating shared key
    }
    \endcode
...
/*!
    ...
    \return 0 Returned on successfully initializing the curve25519_key structure
...
/*!
    ...
    \return ECC_BAD_ARG_E Returns if privSz is not equal to wc_curve25519_size(key).
...
/*!
    ...
    \return ECC_BAD_ARG_E Returned if any of the input parameters are NULL, or the input key’s key size does not match the public or private key sizes
...
/*!
    ...
    ret = wc_curve25519_import_private_raw_ex(&priv, sizeof(priv), pub, sizeof(pub),&key, EC25519_BIG_ENDIAN);
    if (ret != 0) {
    	// error importing keys
    }
    \endcode
...
/*!
    ...
    \return 0 Returned on successfully exporting the private key from the curve25519_key structure
...
/*!
    ...
    \return 0 Returned on successfully importing the public key into the curve25519_key structure
...
parameter does not match the key size of the key structure.
    ...
    \param key pointer to the curve25519_key structure in which to store the key
...
/*!
    ...
    \brief 0 Returned on successfully importing the public key into the curve25519_key structure
    \brief ECC_BAD_ARG_E Returned if the inLen parameter does not match the key size of the key structure
...
/*!
    ...
    \return 0 Returned on successfully exporting the public key from the curve25519_key structure
    \return ECC_BAD_ARG_E Returned if any of the input parameters are NULL
    
    \param key pointer to the curve25519_key structure in from which to export the key
...
/*!
    ...
    ret = wc_curve25519_export_key_raw_ex(&key,priv, &privSz, pub, &pubSz, EC25519_BIG_ENDIAN);
    if (ret != 0) {
    	// error exporting key
    }
    \endcode
...
/*!
    ...
    \return Success Given a valid, initialized curve25519_key structure, returns the size of the key. 
    \return 0 Returned if key is NULL
    
    \param key pointer to the curve25519_key structure in for which to determine the key size
